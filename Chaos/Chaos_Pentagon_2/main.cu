...
unsigned int nx = 2400;
unsigned int ny = 2400;

#define RND (curand_uniform(local_rand_state))

__global__ void init_seed(vec2* seed,int *last,int n,int rnd_seed,shape shape_pts,vec2 img_size,curandState *rand_state) {
	...
	curand_init(rnd_seed, i, 0, &rand_state[i]);
...
__global__ void startChaosRound(vec2 *seed,int *last,int n,shape shape_pts,vec2 img_size,PixelInfo *img,curandState *rand_state){
	int i = threadIdx.x;
	...
	if(last[i] == last [n+i]) {
		while ((rnd+1)%shape_pts.n == last[i] || (rnd-1)%shape_pts.n == last[i]) {
...
int main() {
	...
	int *last;
	checkCudaErrors(cudaMalloc((void **)&last, 2*n*sizeof(int)));
	...
	init_seed<<<blocks,threads>>>(seed,last,n,rnd_seed,shape_pts,{double(nx),double(ny)},d_rand_state);
	...
	for(int i=0;i<itter;i++) {
		...
		startChaosRound<<<blocks,threads>>>(seed,last,n,shape_pts,{double(nx),double(ny)},img,d_rand_state);
		checkCudaErrors(cudaGetLastError());
		checkCudaErrors(cudaDeviceSynchronize());
